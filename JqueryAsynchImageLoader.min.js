...
* Licensed under MIT
*/
(function($){$.fn.asynchImageLoader=function(options){options=$.extend({timeout:10,effect:"fadein",speed:400,selector:"",event:"load",callback:false,placeholder:false},options);var images=this;if(options.placeholder!==false){images.each(function(){$(this).attr("src",options.placeholder);});}images.each(function(index){var parents=$(this).parents();for(var i=0;i<parents.length;i++){if($(parents).get(i).tagName.match(/noscript/ig)){Array.prototype.splice.apply(images,[index,1]);}}});switch(options.event){case"click":$.asynchImageLoader.onEvent.apply(this,Array.prototype.slice.call(arguments));break;case"mouseover":$.asynchImageLoader.onEvent.apply(this,Array.prototype.slice.call(arguments));break;case"scroll":$.asynchImageLoader.onScroll.apply(this,Array.prototype.slice.call(arguments));break;default:$.asynchImageLoader.later.apply(this,[options]);}return images.each(function(){return this;});};$.asynchImageLoader={onEvent:function(options){var images=$(this);if($(options.selector).length===0){$(images).bind(options.event,function(e){if($.data(this,"loaded")!=="true"){$.asynchImageLoader._loadImage(options,this);}if(!options.callback){return false;}return options.callback.call(this,options);});}else{$(options.selector).bind(options.event,function(e){$(images).each(function(){if(!($(this).parent().get(0).tagName.match(/noscript/ig))){if($.data(this,"loaded")!=="true"){$.asynchImageLoader._loadImage(options,this);}}});if(!options.callback){return false;}return options.callback.call(this,options,images);});}},later:function(options){var images=$(this);setTimeout(function(){images.each(function(){$.asynchImageLoader._checkTheImageInTheScreen(options,this);});$(window).bind("scroll",function(){images.each(function(){if($.data(this,"loaded")!=="true"){$.asynchImageLoader._checkTheImageInTheScreen(options,this);}});});},options.timeout);},onScroll:function(options){var images=$(this);$(window).bind("scroll",function(){images.each(function(){$.asynchImageLoader._checkTheImageInTheScreen(options,this);});});},_checkTheImageInTheScreen:function(options,image){if($.data(image,"loaded")==="true"){return;}if($.asynchImageLoader._isInTheScreen(window,image)){$.asynchImageLoader._loadImage(options,image);}},_isInTheScreen:function(windowEl,image){return($(windowEl).scrollTop()<=$(image).offset().top)&&(($(windowEl).scrollTop()+$(windowEl).height())>=($(image).offset().top)&&($(windowEl).scrollLeft()<=$(image).offset().left)&&(($(windowEl).scrollLeft()+$(windowEl).width())>=$(image).offset().left));},_loadImage:function(options,image){if(options.effect.match("/fadein/ig")){$(image).attr("src",$(image).attr("name")).fadeIn(options.speed);}else{$(image).attr("src",$(image).attr("name")).show();}}};}(jQuery));